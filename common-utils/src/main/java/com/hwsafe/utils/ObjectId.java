...
public final class ObjectId implements Comparable<ObjectId>, Serializable {
    ...
    public ObjectId(final Date date, final int machineIdentifier,
        ...
        this(dateToTimestampSeconds(date), machineIdentifier, processIdentifier,
                counter);
    }
    ...
    public ObjectId(final ByteBuffer buffer) {
        ...
        if (buffer.remaining() < 12) {
            throw new IllegalArgumentException(
                    "state should be: buffer.remaining() >=12");
        }

        // Note: Cannot use ByteBuffer.getInt because it depends on tbe buffer's
        // byte order
        // and ObjectId's are always in big-endian order.
        timestamp = makeInt(buffer.get(), buffer.get(), buffer.get(),
                buffer.get());
        machineIdentifier = makeInt((byte) 0, buffer.get(), buffer.get(),
                buffer.get());
        processIdentifier = (short) makeInt((byte) 0, (byte) 0, buffer.get(),
                buffer.get());
    ...
    private static int makeInt(final byte b3, final byte b2, final byte b1,
            final byte b0) {
        // CHECKSTYLE:OFF
        return (((b3) << 24) | ((b2 & 0xff) << 16) | ((b1 & 0xff) << 8)
                | ((b0 & 0xff)));
        // CHECKSTYLE:ON
    }
