...
/**
 * 
 * 项目名称：hcs-multi-tenancy-web 类名称：CronKit 类描述： 创建人：wangk 创建时间：2020年3月19日
 * 上午10:43:14 修改人：wangk 修改时间：2020年3月19日 上午10:43:14 修改备注：
 * 
 * @version
 * 
 */
public class CronKit {
    ...
    // +--------------- second (0 - 59)
    private enum Bounds {
        seconds(0, 0, 59, Calendar.SECOND), minutes(1, 0, 59, Calendar.MINUTE), hours(
                2, 0, 23, Calendar.HOUR_OF_DAY), dom(3, 1, 31,
                Calendar.DAY_OF_MONTH), // Day1
    ...
    private List<String> months = Arrays.asList(new String[] { "JAN", "FEB",
            "MAR", "APR", "MAY", "JUN", "JUL", "AUG", "SEP", "OCT", "NOV",
            "DEC" });
    private List<String> dow = Arrays.asList(new String[] { "SUN", "MON",
            "TUE", "WED", "THU", "FRI", "SAT" });
    ...
    private Long[] parse(String cron) {
        ...
        Assert.isTrue(items.length == 6 || items.length == 7,
                "error： cron length must be 6 or 7 ");
    ...
    private long getField(String field, int index) {
        ...
        if (field.contains(",")) {// 判断一个节点多个条件,递归
            ...
            Assert.isTrue(start >= bound.min, String.format(
                    "Start of range (%d) must above the minimum (%d):%s",
                    start, bound.min, field));
            Assert.isTrue(end <= bound.max, String.format(
                    "End of range (%d) must below  the maximum (%d):%s", end,
                    bound.max, field));
            Assert.isTrue(start <= end, String.format(
                    "Start of range (%d) must below the end of range (%d):%s",
                    start, end, field));
    ...
    private int parseIntOrName(String field, Bounds bound) {
        if (StringUtil.isNumberString(field))
            ...
            if (bound == Bounds.months)
                ...
                throw new RuntimeException("parseIntOrName No match found:"
                        + field + "[" + bound.index + "]");
        }
    }
