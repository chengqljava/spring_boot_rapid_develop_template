 ...
 *         第一个* 代表任意返回类型
 *         第二个* 任一包 
 *         第三个.*(类名需要用一个.占位) 任意字符结尾的类名
 *         第四个 *(..) 代表任意方法 
 */
@Aspect
@Component
public class LogServiceTakeTime {
	private final static Logger LOGER = LoggerFactory.getLogger(LogServiceTakeTime.class);
	...
	private void logParam(String[] paramsArgsName, Object[] paramsArgsValue) {
		...
		for (int i = 0; i < paramsArgsName.length; i++) {
			...
			buffer.append(name + " = ");
			if(value==null||"".equals(value)||StringUtils.isBlank(value.toString())) {
				buffer.append(value + "  ,");
			}else {
