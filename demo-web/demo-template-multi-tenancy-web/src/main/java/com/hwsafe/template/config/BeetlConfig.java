package com.hwsafe.template.config;
...
public class BeetlConfig {
    ...
    public BeetlSpringViewResolver getBeetlSpringViewResolver(
            @Qualifier("beetlViewConfig") BeetlGroupUtilConfiguration beetlGroupUtilConfiguration) {
    ...
    /*
     * @Bean(initMethod = "init", name = "beetlConfig") public
     * BeetlGroupUtilConfiguration getBeetlGroupUtilConfiguration() {
     * BeetlGroupUtilConfiguration beetlGroupUtilConfiguration = new
     * BeetlGroupUtilConfiguration(); try { //ClasspathResourceLoader cploder =
     * new ClasspathResourceLoader(BeetlConfig.class.getClassLoader(),
     * "/views"); ClasspathResourceLoader classpathResourceLoader = new
     * ClasspathResourceLoader();
     * beetlGroupUtilConfiguration.setResourceLoader(classpathResourceLoader);
     * ResourcePatternResolver patternResolver = ResourcePatternUtils
     * .getResourcePatternResolver(new DefaultResourceLoader());
     * beetlGroupUtilConfiguration
     * .setConfigFileResource(patternResolver.getResource(
     * "classpath:beetl.properties"));
     * 
     * 
     * 
     * Map<String, Object> functionPackages = new HashMap<String, Object>();
     * functionPackages.put("user", new GlobalInfo()); 全局参数
     * beetlGroupUtilConfiguration.setFunctionPackages(functionPackages) ;
     * 
     * 
     * } catch (Exception e) { e.printStackTrace(); throw new
     * RuntimeException(e); } // 读取配置文件信息 return beetlGroupUtilConfiguration; }
     * 
     * @Bean(name = "beetlViewResolver") public BeetlSpringViewResolver
     * getBeetlSpringViewResolver(
     * 
     * @Qualifier("beetlConfig") BeetlGroupUtilConfiguration
     * beetlGroupUtilConfiguration) { BeetlSpringViewResolver
     * beetlSpringViewResolver = new BeetlSpringViewResolver();
     * beetlSpringViewResolver.setPrefix("/view/");
     * beetlSpringViewResolver.setSuffix(".html");
     * beetlSpringViewResolver.setContentType("text/html;charset=UTF-8");
     * beetlSpringViewResolver.setOrder(0);
     * beetlSpringViewResolver.setConfig(beetlGroupUtilConfiguration);
     * beetlSpringViewResolver.setCache(false); return beetlSpringViewResolver;
     * }
     */
}
